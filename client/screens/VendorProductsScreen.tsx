      ...
      >
        ...
        {selectMode && (
          <View style={pc.checkOverlay}>
            ...
            >
              {selected && (
                <Feather name="check" size={12} color={AppColors.white} />
              )}
            </View>
          </View>
        )}
        ...
        <View style={pc.info}>
          ...
          <View style={pc.topRow}>
            <ThemedText style={pc.name} numberOfLines={2}>
              {item.name}
            </ThemedText>
            <View style={[pc.statusChip, { backgroundColor: cfg.bg }]}>
              <MaterialCommunityIcons
                ...
                color={cfg.color}
              />
              <ThemedText style={[pc.statusText, { color: cfg.color }]}>
                {cfg.label}
              </ThemedText>
            </View>
          </View>
          ...
          <View style={pc.bottomRow}>
            <ThemedText style={pc.price}>
              {item.price.toLocaleString("ar-IQ")}{" "}
              <ThemedText style={pc.priceUnit}>د.ع</ThemedText>
            </ThemedText>
            {!selectMode && (
          ...
          {item.status === "rejected" && item.rejectionReason ? (
            ...
            >
              <MaterialCommunityIcons
                ...
                color={AppColors.error}
              />
              <ThemedText style={pc.rejectionText} numberOfLines={2}>
          ...
          <View style={cm.btns}>
            ...
            >
              {confirming ? (
                ...
                <ThemedText style={cm.btnText}>{confirmLabel}</ThemedText>
              )}
            </Pressable>
            <Pressable
              ...
              disabled={confirming}
            >
              <ThemedText style={[cm.btnText, { color: AppColors.gray700 }]}>
                إلغاء
              </ThemedText>
            </Pressable>
          </View>
    ...
    <Modal transparent visible={!!result} animationType="fade">
      <View style={cm.overlay}>
        <View style={cm.box}>
          ...
          <ThemedText style={cm.title}>
            {result?.failed === 0 ? "تم الحذف بنجاح" : "اكتملت العملية"}
          </ThemedText>
...
export default function VendorProductsScreen({ navigation }: any) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setProducts(
        (data.products || []).filter((p: Product) => p.status !== "deleted"),
      );
    } catch {
    } finally {
      setLoading(false);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await fetch(
        new URL(`/api/vendor/products/${deleteTarget}`, getApiUrl()).toString(),
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${vendorToken}` },
        },
  ...
  const toggleAvailability = useCallback(
    async (productId: string, currentInStock: boolean) => {
      ...
      try {
        await fetch(
          new URL(
            `/api/vendor/products/${productId}/availability`,
            getApiUrl(),
          ).toString(),
          {
            method: "PATCH",
  ...
  const renderSectionHeader = ({ section }: { section: SectionData }) => {
    ...
    return (
      <View
        style={[
          sh.wrap,
          { backgroundColor: cfg.bg, borderRightColor: cfg.color },
        ]}
      >
        <View style={sh.left}>
          ...
          <ThemedText style={[sh.title, { color: cfg.color }]}>
            {categoryLabel(section.title)}
          </ThemedText>
        </View>
        <View style={[sh.badge, { backgroundColor: cfg.color + "22" }]}>
          <ThemedText style={[sh.badgeText, { color: cfg.color }]}>
            {section.count} منتج
          </ThemedText>
        </View>
      </View>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: AppColors.background }}>
          ...
          {filterOptions.map((f) => {
            ...
            return (
              ...
              >
                ...
                />
                <ThemedText
                  style={[fb.chipText, active && { color: AppColors.white }]}
                >
                  {f.label}
                </ThemedText>
              </Pressable>
        ...
        <SectionList
          ...
          renderItem={({ item }) => (
            <ProductCard
              ...
              onToggleAvailability={() =>
                toggleAvailability(item.id, item.inStock !== false)
              }
          ...
          contentContainerStyle={{
            ...
            paddingTop: 10,
            paddingBottom: selectMode ? tabBarHeight + 88 : tabBarHeight + 16,
            flexGrow: 1,
          }}
        ...
        <View style={[bulk.bar, { bottom: tabBarHeight + 16 }]}>
          ...
          >
            ...
            <ThemedText
              style={[
                bulk.text,
                selectedIds.size === 0 && { color: AppColors.gray400 },
              ]}
            >
              {selectedIds.size > 0
      ...
      <ConfirmModal
        ...
        desc={`سيتم حذف ${selectedIds.size} منتج. هذا الإجراء لا يمكن التراجع عنه.`}
