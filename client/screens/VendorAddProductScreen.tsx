...
export default function VendorAddProductScreen({ navigation }: any) {
  ...
  const pickImageFromGallery = async () => {
    if (imageUris.length >= MAX_IMAGES) { setError(`الحد الأقصى ${MAX_IMAGES} صور`); return; }
    ...
    if (!result.canceled && result.assets[0]) { setImageUris((p) => [...p, result.assets[0].uri]); setError(""); }
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      const validVariants = variants.filter((v) => v.name.trim()).map((v) => ({ id: v.id, name: v.name.trim(), priceAdjustment: parseFloat(v.priceAdjustment) || 0 }));
      const validAddons   = addons.filter((a) => a.name.trim()).map((a) => ({ id: a.id, name: a.name.trim(), price: parseFloat(a.price) || 0 }));
      ...
      if (!res.ok) { setError(data.error || "حدث خطأ في إضافة المنتج"); return; }
  ...
  if (success) {
    return (
      <View style={[s.successWrap, { paddingTop: headerHeight + 40 }]}>
        ...
        >
          <ThemedText style={[s.btnText, { color: AppColors.gray700 }]}>عرض منتجاتي</ThemedText>
        </Pressable>
      </View>
    );
  }
  ...
  return (
    ...
    >
      {isPending && (
        <View style={[s.banner, { backgroundColor: AppColors.warningLight, borderColor: AppColors.warning }]}>
          <MaterialCommunityIcons name="clock-alert-outline" size={18} color={AppColors.warning} />
          <ThemedText style={s.bannerText}>متجرك قيد المراجعة — يمكنك إعداد منتجاتك الآن، وستظهر للزبائن بعد تفعيل حسابك</ThemedText>
        </View>
      )}

      {error ? (
        <View style={[s.banner, { backgroundColor: AppColors.errorLight, borderColor: AppColors.error }]}>
          <Feather name="alert-circle" size={16} color={AppColors.error} />
          <ThemedText style={[s.bannerText, { color: AppColors.error }]}>{error}</ThemedText>
        </View>
      ) : null}
        ...
        {imageUris.length > 0 && (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.thumbRowContent}>
            {imageUris.slice(1).map((uri, i) => {
              const index = i + 1;
              return (
                <View key={uri + index} style={s.thumbWrap} testID={`image-thumb-${index}`}>
      ...
      <View style={s.card}>
        <View style={s.cardHeader}>
          <MaterialCommunityIcons name="text-box-outline" size={16} color={ORANGE} />
      ...
      <View style={s.card}>
        <View style={s.cardHeader}>
          <MaterialCommunityIcons name="currency-usd" size={16} color={ORANGE} />
      ...
      <View style={s.card}>
        <View style={s.cardHeader}>
          <MaterialCommunityIcons name="tag-multiple-outline" size={16} color={ORANGE} />
      ...
      <View style={s.card}>
        <View style={s.cardHeader}>
          <MaterialCommunityIcons name="format-list-bulleted-type" size={16} color={ORANGE} />
          <ThemedText style={[s.cardTitle, { flex: 1 }]}>الأحجام / الخيارات <ThemedText style={s.optional}>(اختياري)</ThemedText></ThemedText>
          <Pressable onPress={addVariant} style={s.addExtraBtn} testID="button-add-variant">
            <Feather name="plus" size={15} color={ORANGE} />
            <ThemedText style={[s.addExtraText, { color: ORANGE }]}>إضافة</ThemedText>
          </Pressable>
        </View>
        {variants.length === 0 ? (
          <ThemedText style={s.hint}>مثال: صغير (+0)، وسط (+2000)، كبير (+5000)</ThemedText>
        ) : null}
        {variants.map((v) => (
          <View key={v.id} style={s.extraRow}>
            <TextInput style={[s.input, s.extraInput, { flex: 2 }]} value={v.name} onChangeText={(val) => updateVariant(v.id, "name", val)} placeholder="الاسم" placeholderTextColor={AppColors.gray300} testID={`input-variant-name-${v.id}`} />
            <TextInput style={[s.input, s.extraInput, { flex: 1 }]} value={v.priceAdjustment} onChangeText={(val) => updateVariant(v.id, "priceAdjustment", val)} placeholder="فرق السعر" placeholderTextColor={AppColors.gray300} keyboardType="numeric" testID={`input-variant-price-${v.id}`} />
            <Pressable onPress={() => removeVariant(v.id)} style={s.extraRemoveBtn} testID={`button-remove-variant-${v.id}`}>
      ...
      <View style={s.card}>
        <View style={s.cardHeader}>
          <MaterialCommunityIcons name="plus-circle-multiple-outline" size={16} color={ORANGE} />
          <ThemedText style={[s.cardTitle, { flex: 1 }]}>الإضافات <ThemedText style={s.optional}>(اختياري)</ThemedText></ThemedText>
          <Pressable onPress={addAddon} style={s.addExtraBtn} testID="button-add-addon">
            <Feather name="plus" size={15} color={ORANGE} />
            <ThemedText style={[s.addExtraText, { color: ORANGE }]}>إضافة</ThemedText>
          </Pressable>
        </View>
        {addons.length === 0 ? (
          <ThemedText style={s.hint}>مثال: جبن إضافي (+2000)، صلصة حارة (+500)</ThemedText>
        ) : null}
        {addons.map((a) => (
          <View key={a.id} style={s.extraRow}>
            <TextInput style={[s.input, s.extraInput, { flex: 2 }]} value={a.name} onChangeText={(val) => updateAddon(a.id, "name", val)} placeholder="اسم الإضافة" placeholderTextColor={AppColors.gray300} testID={`input-addon-name-${a.id}`} />
            <TextInput style={[s.input, s.extraInput, { flex: 1 }]} value={a.price} onChangeText={(val) => updateAddon(a.id, "price", val)} placeholder="السعر" placeholderTextColor={AppColors.gray300} keyboardType="numeric" testID={`input-addon-price-${a.id}`} />
            <Pressable onPress={() => removeAddon(a.id)} style={s.extraRemoveBtn} testID={`button-remove-addon-${a.id}`}>
              <Feather name="x" size={14} color={AppColors.error} />
            </Pressable>
          </View>
        ))}
      ...
      {/* Note */}
      <View style={[s.banner, { backgroundColor: AppColors.secondary, borderColor: ORANGE + "30" }]}>
        <MaterialCommunityIcons name="image-filter-none" size={16} color={ORANGE} />
      ...
      <Pressable style={[s.submitBtn, loading && { opacity: 0.6 }]} onPress={submit} disabled={loading} testID="button-submit">
        {loading
          ...
          : <><Feather name="upload-cloud" size={18} color={AppColors.white} /><ThemedText style={s.submitText}>رفع المنتج للمراجعة</ThemedText></>}
      </Pressable>
    </KeyboardAwareScrollView>
  );
}
...
const s = StyleSheet.create({
  ...
  card:         { backgroundColor: AppColors.white, borderRadius: 20, padding: 16, marginBottom: 14, shadowColor: AppColors.black, shadowOpacity: 0.04, shadowRadius: 8, shadowOffset: { width: 0, height: 2 }, elevation: 2 },
  cardHeader:   { flexDirection: "row-reverse", alignItems: "center", gap: 8, marginBottom: 12 },
  cardTitle:    { fontFamily: FontFamily.cairoBold, fontSize: 15, color: AppColors.gray800, textAlign: "right" },
  ...
  bannerText:   { fontFamily: FontFamily.tajawal, fontSize: 12, color: AppColors.primaryDark, flex: 1, textAlign: "right", lineHeight: 20 },
  ...
  libThumbWrap: { width: 96, height: 96, borderRadius: 14, overflow: "hidden", borderWidth: 2, borderColor: "transparent" },
  libThumb:     { width: "100%", height: "100%" },
  libCheckOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(0,0,0,0.38)", alignItems: "center", justifyContent: "center" },
  libSelectedRow:  { flexDirection: "row-reverse", alignItems: "center", gap: 6, marginTop: 8, paddingHorizontal: 4 },
  libSelectedText: { fontFamily: FontFamily.cairoMedium, fontSize: 12 },
  libDivider:   { flexDirection: "row-reverse", alignItems: "center", gap: 8, marginTop: 12 },
  libLine:      { flex: 1, height: 1, backgroundColor: AppColors.divider },
  libOrText:    { fontFamily: FontFamily.tajawal, fontSize: 12, color: AppColors.gray500 },
  hint:         { fontFamily: FontFamily.tajawal, fontSize: 12, color: AppColors.gray400, textAlign: "right", marginBottom: 8 },
  extraRow:     { flexDirection: "row-reverse", alignItems: "center", gap: 8, marginBottom: 8 },
  extraInput:   { marginBottom: 0, paddingVertical: 10, fontSize: 14 },
  extraRemoveBtn:{ width: 32, height: 32, borderRadius: 16, backgroundColor: AppColors.errorLight, alignItems: "center", justifyContent: "center" },
  addExtraBtn:  { flexDirection: "row-reverse", alignItems: "center", gap: 4, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20, backgroundColor: AppColors.secondary },
  ...
  btnText:      { fontFamily: FontFamily.cairoBold, fontSize: 15, color: AppColors.white },
});
