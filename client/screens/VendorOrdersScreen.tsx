...
function buildReceiptHTML(order: VendorOrder): string {
  ...
  const customerName =
    order.customerName ?? order.customerPhone ?? order.phoneNumber ?? "—";
  ...
  const createdDate = (() => {
    try {
      return new Date(order.createdAt).toLocaleDateString("ar-IQ", {
        year: "numeric",
        month: "long",
        day: "numeric",
      });
  ...
  const createdTime = (() => {
    try {
      return new Date(order.createdAt).toLocaleTimeString("ar-IQ", {
        hour: "2-digit",
        minute: "2-digit",
      });
    } catch {
  ...
  const vendorTotal =
    order.vendorSubtotal ?? order.restaurantSubtotal ?? order.total;
  ...
  return (
    <View style={[chip.wrap, { backgroundColor: cfg.bg }]}>
      <MaterialCommunityIcons
        ...
        color={cfg.color}
      />
      <ThemedText style={[chip.text, { color: cfg.color }]}>
        {cfg.label}
      </ThemedText>
    </View>
  );
...
const StatusTimerBadge = React.memo(
  ({
    ...
    const threshold =
      thresholds[status] ?? DEFAULT_URGENCY_THRESHOLD[status] ?? 15;
        ...
        return (
          <React.Fragment key={step.key}>
            <View style={tl.step}>
              ...
              <ThemedText
                style={[tl.label, { color: done ? ORANGE : AppColors.gray300 }]}
              >
                {step.label}
              </ThemedText>
            </View>
            {i < TIMELINE_STEPS.length - 1 && (
              <View
                style={[
                  tl.line,
                  { backgroundColor: i < active ? ORANGE : AppColors.gray200 },
                ]}
              />
            )}
          </React.Fragment>
        ...
        <View style={{ flex: 1 }}>
          ...
          <ThemedText style={ds.sub}>
            {order.status === "picked_up"
              ? "استلم الطلب · في الطريق للزبون"
              : "في الطريق إلى الزبون"}
          </ThemedText>
          {order.driverPhone ? (
            <ThemedText style={ds.phone}>{order.driverPhone}</ThemedText>
          ) : null}
        </View>
        <MaterialCommunityIcons
          name="navigation"
...
const OrderCard = React.memo(
  ({
    ...
    const vendorTotal =
      order.vendorSubtotal ?? order.restaurantSubtotal ?? order.total;
    const displayName =
      order.customerName || order.customerPhone || order.phoneNumber || "عميل";
      ...
      >
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={oc.metaRow}>
              ...
              <ThemedText style={oc.timeText}>
                {relativeTime(order.createdAt)}
              </ThemedText>
              <ThemedText style={oc.dot}>·</ThemedText>
              <ThemedText style={oc.itemCount}>
          ...
          <View style={oc.headerRight}>
            <ThemedText style={oc.totalText}>
              {order.total.toLocaleString("ar-IQ")}
            </ThemedText>
        ...
        {expanded && (
          <>
            ...
            {/* Divider */}
            <View
              style={[oc.divider, { backgroundColor: AppColors.gray100 }]}
            />
            ...
            <View style={oc.section}>
              ...
              {order.address ? (
                <View style={oc.infoRow}>
                  ...
                  <ThemedText style={oc.infoText} numberOfLines={2}>
                    {order.address}
                  </ThemedText>
                </View>
              ) : null}
                ...
                <View
                  style={[
                    oc.notesBox,
                    {
                      ...
                      borderColor: AppColors.warning + "50",
                    },
                  ]}
                >
                  <MaterialCommunityIcons
                    ...
                    color={AppColors.warning}
                  />
                  <ThemedText style={oc.notesText} numberOfLines={3}>
                    {order.notes}
                  </ThemedText>
                </View>
              ) : null}
              ...
              {Array.isArray(order.items) &&
                order.items.map((item, i) => (
                  ...
                  >
                    ...
                    <View
                      style={[oc.qtyBadge, { backgroundColor: ORANGE + "15" }]}
                    >
                      <ThemedText style={[oc.qtyText, { color: ORANGE }]}>
                        ×{item.quantity}
                      </ThemedText>
                    </View>
                    <ThemedText style={oc.itemName} numberOfLines={2}>
                      {item.name}
                    </ThemedText>
            ...
            <View
              style={[
                oc.breakdown,
                { backgroundColor: ORANGE + "06", borderColor: ORANGE + "15" },
              ]}
            >
              <CostRow
                icon="store-outline"
              ...
              <View style={oc.actionsRow}>
                {actions.map((a) => (
                  ...
                  >
                    ...
                    <ThemedText style={[oc.actionText, { color: a.color }]}>
                      {a.label}
                    </ThemedText>
                  </Pressable>
                ))}
    ...
    <View style={cr.row}>
      ...
      <View style={{ flex: 1 }}>
        <ThemedText
          style={[
            cr.label,
            bold && { fontFamily: FontFamily.cairoBold },
    ...
    <Modal transparent visible={visible} animationType="slide">
      <View style={md.overlay}>
        <View style={md.sheet}>
          ...
          <View
            style={[md.iconCircle, { backgroundColor: AppColors.successLight }]}
          >
            <MaterialCommunityIcons
              name="clock-outline"
              size={32}
          ...
          <ThemedText style={md.subtitle}>
            كم تحتاج من الوقت لتحضير هذا الطلب؟
          </ThemedText>
          <View style={md.optionRow}>
            {ETA_OPTIONS.map((mins) => {
              ...
              return (
                ...
                >
                  <ThemedText
                    style={[md.optionVal, sel && { color: AppColors.success }]}
                  >
                    {mins}
                  </ThemedText>
                  <ThemedText
                    style={[md.optionUnit, sel && { color: AppColors.success }]}
                  >
                    د
                  </ThemedText>
                </Pressable>
          ...
          >
            <ThemedText style={[md.btnText, { color: AppColors.gray700 }]}>
              تراجع
            </ThemedText>
          </Pressable>
        </View>
    ...
    <Modal transparent visible={visible} animationType="slide">
      <View style={md.overlay}>
        <View style={md.sheet}>
          <View style={md.handle} />
          <View
            style={[md.iconCircle, { backgroundColor: AppColors.errorLight }]}
          >
            <MaterialCommunityIcons
...
const EmptyState = React.memo(({ tab }: { tab: (typeof TABS)[0] }) => {
  const messages: Record<string, { title: string; sub: string }> = {
    new: {
      ...
      sub: "ستظهر هنا الطلبات الجديدة فور وصولها",
    },
    active: {
      title: "لا طلبات قيد التحضير",
      sub: "ستنتقل الطلبات المؤكدة هنا تلقائياً",
    },
    ready: {
      title: "لا طلبات جاهزة الآن",
      sub: "ستظهر الطلبات الجاهزة للاستلام هنا",
    },
    done: {
      title: "لا طلبات مكتملة بعد",
      sub: "ستظهر الطلبات المكتملة والملغاة هنا",
    },
  };
...
export default function VendorOrdersScreen() {
  ...
  const [urgencyThresholds, setUrgencyThresholds] = useState<
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(
          new URL("/api/settings/urgency-thresholds", getApiUrl()).toString(),
        );
        if (res.ok) {
          const data = await res.json();
  ...
  const updateStatus = useCallback(
    async (orderId: string, newStatus: string, estimatedMinutes?: number) => {
      if (!vendorToken) return;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: AppColors.background }}>
                ...
                {count > 0 && (
                  <View
                    style={[
                      ...
                      {
                        backgroundColor:
                          t.key === "new" ? AppColors.error : t.color,
                      },
                    ]}
                  >
                    <ThemedText style={ts.badgeText}>
                      {count > 9 ? "9+" : count}
                    </ThemedText>
                  </View>
                )}
      ...
      <ETAModal
        visible={!!etaTarget}
        onConfirm={(mins) => {
          if (etaTarget) updateStatus(etaTarget, "confirmed", mins);
          setEtaTarget(null);
        }}
