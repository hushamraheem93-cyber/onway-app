...
export default function VendorEditProductScreen({ navigation, route }: any) {
  ...
  const pickImageFromGallery = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      setImages((p) => [...p, { type: "new", uri: result.assets[0].uri }]);
      setChanged(true);
      setError("");
    }
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (imagesChanged) {
        ...
        for (const img of images.filter(
      ...
      const res = await fetch(
        new URL(`/api/vendor/products/${product.id}`, getApiUrl()).toString(),
        {
          method: "PUT",
  ...
  if (success) {
    return (
      <View style={[s.successWrap, { paddingTop: headerHeight + 40 }]}>
        ...
        >
          <ThemedText style={[s.submitText, { color: AppColors.gray700 }]}>
            عرض منتجاتي
          </ThemedText>
        </Pressable>
      </View>
    ...
    >
      ...
      <View style={s.sectionCard}>
        <View style={s.sectionHeader}>
          ...
          <ThemedText style={s.sectionHint}>
            (حتى {MAX_IMAGES} صور، الأولى رئيسية)
          </ThemedText>
        </View>
        ...
        {images.length > 0 && (
          ...
          >
            {images.slice(1).map((img, i) => {
              const index = i + 1;
              const uri =
                img.type === "existing" ? resolveImageUrl(img.url) : img.uri;
              return (
                <View
                  key={`${img.type}-${index}`}
                  style={s.thumbWrap}
        ...
        {imagesChanged && (
          ...
          >
            ...
            />
            <ThemedText
              style={[s.noticeText, { color: AppColors.primaryDark }]}
            >
      ...
      >
        {loading ? (
          ...
          <>
            ...
            <ThemedText style={s.submitText}>حفظ التعديلات</ThemedText>
          </>
        )}
      </Pressable>
    </KeyboardAwareScrollView>
