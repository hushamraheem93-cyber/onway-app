  ...
  ({
    ...
    return (
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          <ThemedText style={[sr.title, danger && { color: AppColors.error }]}>
            {title}
          </ThemedText>
          {subtitle ? (
            <ThemedText style={[sr.sub, { color: theme.textSecondary }]}>
              {subtitle}
            </ThemedText>
          ) : null}
        </View>
...
export default function VendorProfileScreen() {
  ...
  const { vendorProfile, vendorToken, logout, refreshVendorProfile } =
    useAuth();
  ...
  const [settDeliveryTime, setDeliveryTime] = useState(
    vendorProfile?.deliveryTime || "30-45",
  );
  const [settDeliveryPrice, setDeliveryPrice] = useState(
  ...
  const uploadImage = async (type: "profileImage" | "coverImage") => {
    ...
    const setter =
      type === "profileImage" ? setUploadingAvatar : setUploadingCover;
    setter(true);
    try {
  ...
  const openLocationPicker = () => {
    ...
    navigation.navigate("MapPicker", {
      ...
      onPicked: (loc) => {
        saveStoreLocation(loc.latitude, loc.longitude);
      },
    });
  };
  ...
  const saveStoreLocation = async (latitude: number, longitude: number) => {
    ...
    try {
      ...
      Alert.alert(
        "تعذّر الحفظ",
        "لم يتم حفظ موقع المتجر. تحقق من الاتصال وحاول مرة أخرى.",
      );
    } finally {
  ...
  const saveBio = async () => {
    ...
    try {
      ...
      Alert.alert(
        "تعذّر الحفظ",
        "لم يتم حفظ وصف المتجر. تحقق من الاتصال وحاول مرة أخرى.",
      );
    } finally {
      setSavingBio(false);
    }
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      Alert.alert(
        "تعذّر الحفظ",
        "لم يتم حفظ إعدادات المتجر. تحقق من الاتصال وحاول مرة أخرى.",
      );
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <View style={p.editCoverBtn}>
            {uploadingCover ? (
              ...
              <Feather name="camera" size={13} color={AppColors.white} />
            )}
          </View>
        </Pressable>
        ...
        <View style={p.avatarRow}>
          ...
          >
            ...
            <View style={p.editAvatarBtn}>
              {uploadingAvatar ? (
                ...
                <Feather name="camera" size={11} color={AppColors.white} />
              )}
            </View>
          </Pressable>
          ...
          <View style={{ flex: 1, gap: 4 }}>
            ...
            >
              <ThemedText style={p.storeName} numberOfLines={2}>
            ...
            <ThemedText style={[p.bizType, { color: theme.textSecondary }]}>
              {BUSINESS_LABELS[(vendorProfile as any)?.businessType || ""] ||
                "متجر"}
            </ThemedText>
            <View style={[p.statusBadge, { backgroundColor: statusCfg.bg }]}>
              ...
              />
              <ThemedText style={[p.statusText, { color: statusCfg.color }]}>
                {statusCfg.label}
              </ThemedText>
            </View>
          </View>
        ...
        {vendorProfile?.address && (
          <View style={p.addrRow}>
            ...
            <ThemedText style={[p.addrText, { color: theme.textSecondary }]}>
              {vendorProfile.address}
            </ThemedText>
          </View>
        )}
      ...
      <SettingsRow
        ...
        subtitle={
          [
            vendorProfile?.deliveryTime
              ? `${vendorProfile.deliveryTime} دقيقة`
              : null,
            vendorProfile?.deliveryPrice != null
              ? `أجرة التوصيل: ${vendorProfile.deliveryPrice} د.ع`
              : null,
          ]
            .filter(Boolean)
            .join(" · ") || "غير محدد"
      ...
      <SettingsRow
        ...
        iconBg={AppColors.vendorPurpleLight}
        iconColor={AppColors.vendorPurple}
        title="موقع المتجر على الخريطة"
        subtitle={
          typeof (vendorProfile as any)?.latitude === "number" &&
          typeof (vendorProfile as any)?.longitude === "number"
        ...
        rightNode={
          savingLocation ? (
            <ActivityIndicator size="small" color={AppColors.vendorPurple} />
          ) : undefined
        }
      />
      ...
      {vendorProfile?.rating != null && (
        <SettingsRow
          ...
          rightNode={
            ...
            >
              {(vendorProfile.rating as number).toFixed(1)}
            </ThemedText>
          }
        />
      )}
      ...
      {/* ── Store Name Modal ── */}
      <SheetModal
        visible={storeNameModal}
        title="اسم المتجر"
        onClose={() => setStoreNameModal(false)}
      >
        <TextInput
          style={[
            md.input,
        ...
        <View style={md.btns}>
          <Pressable
            style={[
              md.btn,
              {
                backgroundColor: !storeNameText.trim()
                  ? AppColors.gray200
                  : ORANGE,
              },
            ]}
          ...
          >
            {savingStoreName ? (
              ...
              <ThemedText style={md.btnText}>حفظ</ThemedText>
            )}
          </Pressable>
          <Pressable
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          >
            <View
              style={[
                md.toggle,
                { backgroundColor: useHours ? ORANGE : AppColors.divider },
              ]}
            >
              ...
              {DAY_LABELS.map((d, i) => {
                ...
                return (
                  <Pressable
                    ...
                    style={[
                      md.dayBtn,
                      on && { backgroundColor: ORANGE, borderColor: ORANGE },
                    ]}
                    onPress={() =>
                      setOpenDays((p) =>
                  ...
                  >
                    <ThemedText
                      style={[md.dayText, on && { color: AppColors.white }]}
                    >
                      {d}
                    </ThemedText>
                  </Pressable>
