...
- **App Config:** `app.config.js` replaces `app.json` to support env vars at build time. Bundle IDs updated to `com.husham.onway` (iOS & Android). Google Maps API key injected via `process.env.GOOGLE_MAPS_API_KEY` for both iOS (`config.googleMapsApiKey`) and Android (`config.googleMaps.apiKey`).
- **TypeScript Health:** Zero client TypeScript errors. All `server/routes.ts` and `server/vendor.ts` type errors fixed (string|string[] casts, FirestoreVendor interface, DriverActivityType union). Client fixes: ErrorFallback monospace font, OrderCard border color, VendorTabNavigator screenOptions type, SupportChatScreen getApiUrl import, AdminScreen theme/ThemedText types, MapPickerScreen addressInput style.
