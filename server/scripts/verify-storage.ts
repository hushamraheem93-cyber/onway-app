...
import { getFirestore, uploadToFirebaseStorage, deleteFromFirebaseStorage } from "../firebase";
...
async function main() {
  ...
  if (!db) fail("Admin SDK did not initialise — check FIREBASE_SERVICE_ACCOUNT.");
  ...
  if (!exists) fail(`bucket ${bucket.name} does not exist or the service account cannot see it.`);
  ...
  try {
    step(4, "Uploading through uploadToFirebaseStorage() — the real upload path");
    ...
    if (url.startsWith("data:")) fail("a Base64 data URI came back — the Storage upload did not happen.");
    if (!url.startsWith("https://firebasestorage.googleapis.com/")) {
      fail(`unexpected URL shape: ${url}`);
    }
    if (!url.includes(`/b/${bucket.name}/`)) fail(`URL points at a different bucket: ${url}`);
    if (!url.includes("token=")) fail("URL carries no download token — it would not be fetchable.");
    ...
    if (!got.equals(PNG_1x1)) fail(`round-trip mismatch: sent ${PNG_1x1.length}B, got ${got.length}B`);
    console.log(`    ✓ ${got.length} bytes returned, identical to what was uploaded`);
